...
//global variables
int intensity = 2;                      // Default intensity/brightness (0-15)
...
void display_thp()
{
  ...
  byte offset = 0;
  while (temp[i]) {
    putnormalchar(i * 6 + 2 - offset, 0, temp[i], font_style, font_cols);
    i++;
    if (i == 3) {
      offset = 4;
    }
  }
  levelbar (i * 6 - 2, 0, 2, 1);
  putnormalchar(i * 6 + 2 - 1, 0, 'C', font_style, font_cols);
  ...
  i = 0;
  while (humi[i]) {
    putnormalchar(i * 6 + 2, 0, humi[i], font_style, font_cols);
    i++;
  }
  putnormalchar(i * 6 + 2, 0, '%', font_style, font_cols);
  ...
  i = 0;
  while (pres[i]) {
    puttinychar(i * 4 + 4, 1, pres[i]);
    i++;
  }
  byte x = 0;
  char mb[3] = "mb";
  while (mb[x]) {
    puttinychar(i * 4 + 4, 1, mb[x]);
    x++;i++;
  }
...
void switch_mode() {
  ...
  for (int count = 0; count < 35 ; count++) {
    ...
    if (buttonA.uniquePress() || firstrun == 1) {
      ...
      byte i = 0;
      while (str_top[i]) {
        putnormalchar(i * 6, 0, str_top[i], font_style, font_cols);
        i++;
      }
      firstrun = 0;
    }
